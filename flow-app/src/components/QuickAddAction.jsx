    ...
    <form onSubmit={handleAddAction} className="bg-white/80 backdrop-blur-sm rounded-lg border border-self/20 p-2">
      <div className="flex gap-2 items-center">
        <input
          ...
          className="flex-1 px-2 py-1.5 text-xs border border-gray-200 rounded focus:border-self focus:outline-none bg-white"
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              e.preventDefault();
              handleAddAction();
        ...
        <input
          ...
          min="1"
          max="180"
          className="w-14 px-2 py-1.5 text-xs text-center border border-gray-200 rounded focus:border-self focus:outline-none bg-white"
        />
        <button
          type="submit"
