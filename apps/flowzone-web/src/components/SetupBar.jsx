...
const SetupBar = () => {
  ...
  return (
    <>
      <div className="overflow-visible card-glass-space border-0 rounded-none relative">
        {/* Ambient space glow */}
        <div className="absolute inset-0 bg-glow-space opacity-30 pointer-events-none" />
        ...
        <div className="px-6 py-2.5 flex items-center gap-3 relative z-10">
          <img
            ...
            style={{ filter: 'drop-shadow(0 0 8px rgba(107, 162, 146, 0.5))' }}
          />
          ...
          <div className="flex items-center gap-2 relative">
            ...
            <div ref={soundRef} className="relative">
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1 rounded-lg border transition-all ${
                  showSoundDropdown || saveAnimation === 'sound'
                    ? 'border-space bg-space/30 shadow-md shadow-space/20 scale-105'
                    : 'border-space/30 bg-glass hover:border-space/50 hover:bg-glass-light'
                }`}
              >
              ...
              {/* Sound Dropdown */}
              {showSoundDropdown && (
                <div className="absolute top-full mt-2 right-0 bg-darker/95 backdrop-blur-lg rounded-xl shadow-xl border border-glass-border py-2 min-w-[160px] z-50 animate-fade-in">
                  <div className="px-3 py-1.5 border-b border-glass-border">
                      ...
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-xs transition-colors ${
                          settings.sound === option.value
                            ? 'bg-space/20 text-space font-semibold'
                            : 'text-ivory/70 hover:bg-glass-light hover:text-ivory'
                        }`}
                      >
                        {option.label}
                        {settings.sound === option.value && (
                  ...
                  <div className="px-3 py-2 border-t border-glass-border">
                    <button
                      ...
                      className="w-full text-[10px] text-ivory/40 hover:text-ivory font-medium"
                    >
                      Close
                    </button>
                  </div>
            ...
            <div ref={breathworkRef} className="relative">
              ...
              >
                ...
                <span className="text-[9px] text-ivory/60">{breathworkText}</span>
              </button>

              {/* Breathwork Dropdown */}
              {showBreathworkDropdown && (
                <div className="absolute top-full mt-2 right-0 bg-darker/95 backdrop-blur-lg rounded-xl shadow-xl border border-glass-border py-2 min-w-[200px] z-50 animate-fade-in">
                  {/* Before Section */}
                  <div className="px-3 py-1.5 border-b border-glass-border">
                  ...
                  {/* After Section */}
                  <div className="px-3 py-1.5 border-b border-t border-glass-border mt-1">
